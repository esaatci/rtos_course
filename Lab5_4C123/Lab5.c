...
int main(void){
  ...
  if(BSP_Button1_Input() == 0){ // run TExaS if Button1 is pressed
    ...
    TExaS_Init(GRADER, 2896);   // initialize the Lab 5 grader
...
/*
 ...
 int main(void) {
   ...
   BSP_Clock_InitFastest();
   eDisk_WriteSector(my_buff, 8);
   eDisk_ReadSector(read_buff, 8);
   result = eDisk_Format();
   eDisk_ReadSector(buff_two, 8);
