...
typedef void (*peridicThread) (void);
...
ptcb_t periodicThreads[NUM_PERIODIC_THREADS];
...
int OS_AddPeriodicEventThreads(void(*thread1)(void), uint32_t period1,
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
  periodicThreads[0].pt = thread1;
  periodicThreads[0].period = period1;

  periodicThreads[1].pt = thread2;
...
void Scheduler(void){ // every time slice
  ...
  if (counter % 1 != 99) {
    periodicThreads[0].pt();
  }
  if (counter % 100 == 99) {
    periodicThreads[1].pt();
  }
  RunPt = RunPt->next;
